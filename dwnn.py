...
def preProcessingData(data):
    ...
    df = pd.DataFrame(data, columns = ['xCoordinate', 'yCoordinate'])
    return df
...
def dwnn(dataset, x, sigma):
    ...
    for z in range(len(dataset)):
        xi, yi = dataset.loc[z, 'xCoordinate'], dataset.loc[z, 'yCoordinate']
        u, d = u + (yi*weight(xi, x, sigma)), d + weight(xi, x, sigma)
    y = u/d
    plt.scatter(dataset.loc[:, 'xCoordinate'], dataset.loc[:,'yCoordinate'], color='blue')
